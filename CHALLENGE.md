...
**Goal:** Find potential duplicate accounts by analyzing cross-table patterns (IP addresses, orders, activity).

**Data Characteristics:**
- 50+ users per shared IP address (ws_user_activity table)
- Users with identical order history (ws_orders table)
- Activity timestamps revealing login patterns
- Phone/email duplicates (ws_user table)
...
- Users with same IP address → likely duplicates (same device/location)
- IP data stored in activity logs, not user table
- Must JOIN tables to link IP → user info
- GROUP BY and aggregate to find IPs with multiple users
...
```json
{
  ...
  "duplicate_groups": [
    ...
    {
      "group_id": 2,
      "shared_attribute": "192.168.222.169",
      "attribute_type": "ip_address",
      "user_count": 57,
      "user_ids": [6934182, 7798837, 13514166, ...],
      "user_names": [...],
      "first_activity": "2026-05-18T22:32:01Z",
      "last_activity": "2026-08-16T02:46:27Z",
      "confidence": "high"
    }
  ],
  "total_groups_found": 2,
  "total_duplicate_users": 114
}
```
...
**Detection Methods (Pick One or Combine):**

1. **IP Address Clustering:**
   - Find users sharing same IP address
   - HIGH confidence (same device/location)

2. **Order History:**
   - Find users with matching order patterns
   - MEDIUM confidence

3. **Activity Patterns:**
   - Analyze login/activity timestamps
   - LOW confidence (can vary)

**Scoring:**
- Combine methods via weighted scoring
- HIGH > MEDIUM > LOW confidence
- Return top N duplicates by score
...
**FINAL BOSS** — Prove your API handles multi-table JOINs under load.

**Test Specifications:**

- **Duration:** 60 seconds
- **Concurrency:** 100 concurrent connections
- **Primary Query:** Cross-table JOIN (user profile + orders + transactions + activity)
  - 100% of requests: GET /api/user-profile/:user_id
...
- Order count + total transaction amount
- Activity count + last activity timestamp
- Data from: user + orders + transactions + activity logs

**Architecture Hint:**
- 4-table JOIN required (user, orders, transactions, activity)
- LEFT JOIN to handle users with no orders/activity
- Aggregate functions for counts/sums
